...
static ParseContext *parse_context_new(void)
{
  ...
  ctx->nodes = g_ptr_array_new_with_free_func((GDestroyNotify)fmt_repl_free);
  return ctx;
}
...
static void parse_context_free(ParseContext *ctx)
{
  if (!ctx)
    return;
  if (ctx->stack)
    g_queue_free_full(ctx->stack, (GDestroyNotify)fmt_repl_free);
  if (ctx->nodes)
