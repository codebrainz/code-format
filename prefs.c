...
static void clone_prefs(struct FmtPreferences *psrc, struct FmtPreferences *pdst)
{
...
static void open_user_prefs(void)
{
  ...
  fn = g_build_filename(geany_data->app->configdir, "plugins", "code-format.conf", NULL);
  ...
  kf = g_key_file_new();
  g_key_file_load_from_file(kf, fn,
...
void fmt_prefs_save_user(void)
{
  ...
  char *contents;
  size_t length=0;
  char *fn = g_build_filename(geany_data->app->configdir,
      "plugins", "code-format.conf", NULL);
...
#define SET_WIDGET(parent, name, wid) g_object_set_data(G_OBJECT(parent), name, wid)
...
// Create a .clang-format document based on selected style when Create is clicked
  ...
  G_GNUC_UNUSED gpointer user_data)
{
  ...
  if (str) {
    GeanyDocument *doc = document_new_file(".clang-format",
      filetypes[GEANY_FILETYPES_YAML], str->str);
...
void fmt_prefs_save_panel(GtkWidget *panel, bool project)
{
  ...
  p->style = fmt_style_from_name(gtk_combo_box_get_active_id(GTK_COMBO_BOX(w_style)));
  ...
  if (p == &user_prefs)
    ...
    size_t length=0;
    // Load old contents
    g_key_file_load_from_file(kf, geany_data->app->project->file_name,
      G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, NULL);
    // Update with new contents
    ...
    if (contents) {
      ...
      g_file_set_contents(geany_data->app->project->file_name,
...
GtkWidget *fmt_prefs_create_panel(bool project)
{
  ...
  gtk_entry_set_icon_activatable(GTK_ENTRY(ent), GTK_ENTRY_ICON_SECONDARY, false);
  validate_clang_path_entry(GTK_ENTRY(ent));
  g_signal_connect(ent, "changed", G_CALLBACK(on_pref_clang_format_path_changed), NULL);
  gtk_grid_attach(GTK_GRID(grid), box, 1, row, 2, 1);
  gtk_widget_set_hexpand(box, true);
  gtk_widget_set_tooltip_text(ent, _("Specify the path to the "
  ...
  gtk_widget_set_tooltip_text(combo, _("Select a preset code style "
  ...
  gtk_combo_box_set_active_id(GTK_COMBO_BOX(combo), fmt_style_get_name(p->style));
  ...
  gtk_widget_set_tooltip_text(btn, _("If you press this button, "
  ...
  g_signal_connect(chk, "toggled", G_CALLBACK(on_pref_auto_format_toggled), NULL);
  gtk_widget_set_tooltip_text(chk, _("When auto-formatting is enabled, "
  ...
  gtk_widget_set_tooltip_text(ent, _("When auto-formatting is "
